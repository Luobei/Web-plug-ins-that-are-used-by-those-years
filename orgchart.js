...
(function(factory) {
  ...
  $.fn.orgchart = function(options) {
    var defaultOptions = {
      ...
      'parentNodeSymbol': 'icon-guanliyuan',
      'draggable': false,
      'direction': 't2b',
      'pan': false,
      'zoom': false
  ...
  function showSiblings($node, direction) {
    ...
    $siblings.find('.node:visible').addClass('slide').removeClass('slide-left slide-right').eq(-1).one('transitionend', function() {
      ...
      if (isInAction($node)) {
        switchHorizontalArrow($node);
        $node.children('.topEdge').removeClass('icon-zhankai-copy').addClass('icon-icon-close');
      }
    });
  }
  ...
  function switchVerticalArrow($arrow) {
    $arrow.toggleClass('icon-zhankai-copy').toggleClass('icon-icon-close');
  }
  ...
  function createNode(nodeData, level, opts) {
    ...
    //   $nodeDiv.append('<i class="edge verticalEdge topEdge iconfont"></i>');
    // }
    // if(Number(flags.substr(1,1))) {
    //   $nodeDiv.append('<i class="edge horizontalEdge rightEdge iconfont"></i>' +
    //     '<i class="edge horizontalEdge leftEdge iconfont"></i>');
    // }
    if(Number(flags.substr(2,1))) {
        $nodeDiv.append('<i class="edge verticalEdge bottomEdge iconfont icon-icon-close"></i>');
    }
    $nodeDiv.children('.content').prepend('<i class="iconfont '+ opts.parentNodeSymbol + ' symbol"></i>');
    var done_text = '总公司';
    if(nodeData.className == 'department'){
        done_text = '事业部';
    }else if(nodeData.className == 'branch'){
        done_text = '分公司';
    }
    $nodeDiv.append('<div class="done-btns">'+
                    '	<p>'+done_text+'</p>'+
                    '	<p><a href="javascript:;" title="编辑部门" onclick="dynamic.edit_org_fn('+nodeData.id+')"><i class="iconfont icon-bianji"></i></a><a href="javascript:;" title="添加子部门" onclick="dynamic.add_org_fn('+nodeData.id+')"><i class="iconfont icon-tianjia"></i></a><a href="javascript:;" title="删除部门" onclick="dynamic.del_org_fn('+nodeData.id+')" style="margin-right:0px;"><i class="iconfont icon-shanchu"></i></a></p>'+
                    '</div>');
    $nodeDiv.find('.title,.content').on('mouseover', function(event) {
         var $node = $(this).parent();
         $node.addClass('on');
       })
    $nodeDiv.on('mouseleave', function(event) {
         var $node = $(this);
         $node.removeClass('on');
    ...
    //       $topEdge.toggleClass('icon-zhankai-copy', !flag).toggleClass('icon-icon-close', flag);
    //     }
    //     if ($bottomEdge.length) {
    //       flag = getNodeState($node, 'children').visible;
    //       $bottomEdge.toggleClass('icon-icon-close', !flag).toggleClass('icon-zhankai-copy', flag);
    //     }
    //     if ($leftEdge.length) {
    //       switchHorizontalArrow($node);
    //     }
    //   } else {
    //     $node.children('.edge').removeClass('icon-zhankai-copy icon-icon-close fa-chevron-right fa-chevron-left');
    //   }
    });
    ...
    $nodeDiv.on('click', '.bottomEdge', function(event) {
      ...
      if (childrenState.exist) {
        ...
        if (childrenState.visible) {
          ...
          $that.attr('class','edge verticalEdge bottomEdge iconfont icon-zhankai-copy');
        } else { // show the descendants
          showDescendants($node);
          $node.parent().parent().parent().find('.bottomEdge').attr('class','edge verticalEdge bottomEdge iconfont icon-zhankai-copy');
          $that.attr('class','edge verticalEdge bottomEdge iconfont icon-icon-close');
        }
    ...
    if (opts.draggable) {
      ...
      .on('drop', function(event) {
        ...
        if (!$dropZone.closest('tr').siblings().length) { // if the drop zone is a leaf node
          $dropZone.append('<i class="edge verticalEdge bottomEdge iconfont"></i>')
          ...
          var horizontalEdges = '<i class="edge horizontalEdge rightEdge iconfont"></i><i class="edge horizontalEdge leftEdge iconfont"></i>';
    ...
    buildChildNode.call($node.closest('.orgchart').parent(), $node.closest('table'), data, opts, function() {
      if (++count === data.children.length) {
        ...
        if (!$node.find('.symbol').length) {
          $node.children('.content').prepend('<i class="iconfont '+ opts.parentNodeSymbol + ' symbol"></i>');
        }
        showDescendants($node);
      }
    });
  ...
  function addParent($currentRoot, data, opts) {
    buildParentNode.call(this, $currentRoot, data, opts, function() {
      if (!$currentRoot.children('.topEdge').length) {
        $currentRoot.children('.title').after('<i class="edge verticalEdge topEdge iconfont"></i>');
      }
  ...
  function addSiblings($node, data, opts) {
    buildSiblingNode.call($node.closest('.orgchart').parent(), $node.closest('table'), data, opts, function() {
      ...
      if (!$node.children('.leftEdge').length) {
        $node.children('.topEdge').after('<i class="edge horizontalEdge rightEdge fa"></i><i class="edge horizontalEdge leftEdge iconfont"></i>');
      }
      showSiblings($node);
    });
  }
